...
|  | login_id | NOT NULL, UNIQUE | ログインに必須（NOT NULL）であり、同じIDの人が複数いると誰がログインしたか判別できなくなるため（UNIQUE）。 |
|  | password | NOT NULL | 認証システムにおいてパスワードなしでのログインを防ぐため。 |
...
| product_categories（商品×カテゴリ） | id | AUTO_INCREMENT | （同上）システムが自動で連番を振るため。 |
|  | product_id | NOT NULL | 「どの商品か」という紐付けの根幹データであり、必須のため。 |
|  | category_id | NOT NULL | 「どのカテゴリか」という紐付けの根幹データであり、必須のため。 |
